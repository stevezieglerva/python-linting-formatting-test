...
class EventContext:
    ...
    def _get_last_occurrences(self):
        ...
        results = s3.list_objects_v2(
            Bucket=self.bucket, Prefix="events/last-occurrence"
        )
        file_list = []
        file_list_sorted = []
        for file in results["Contents"]:
            ...
            try:
                ...
                print(
                    f"Error: Unable to parse json from {key}. Skipping looking for the enrichment_summary field"
                )
            file_list.append(new_item)
            file_list_sorted = sorted(
                file_list, key=lambda i: i["last_modified"], reverse=True
            )
    ...
    def _format_last_occurrences_text(self):
        ...
        for event in self.last_occurrences_list:
            ...
            desc = (
                event["key"].replace("events/last-occurrence/", "").replace("__", " ")
            )
