...
def get_metric_dimensions_from_alarm(alarm_name):
    ...
    if len(alarms["MetricAlarms"]) == 1:
        ...
        if "MetricName" in alarm_info:
            ...
            metric_name = alarm_info["Metrics"][metric_position_index]["MetricStat"][
                "Metric"
            ]["MetricName"]
            metric_namespace = alarm_info["Metrics"][metric_position_index][
                "MetricStat"
            ]["Metric"]["Namespace"]
            dimensions = alarm_info["Metrics"][metric_position_index]["MetricStat"][
                "Metric"
            ]["Dimensions"]
            statistic = alarm_info["Metrics"][metric_position_index]["MetricStat"][
                "Stat"
            ]
...
def get_metric_statistics(metric_details):
    ...
    for i in range(0, 5):
        ...
        api_results = cloudwatch.get_metric_statistics(
            Namespace=namespace,
            MetricName=metric,
