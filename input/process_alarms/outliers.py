...
class Outliers():
	...
	def _get_stats_on_list(self, data, num_of_stdevs, periods_of_anomaly=2):
		...
		stats["above_threshold_runs"] = self._get_threshold_runs(above_threshold, periods_of_anomaly)
		stats["currently_in_above_threshold_run"] = self._is_metric_in_threshold_run_now(len(data), stats["above_threshold_runs"])

		stats["below_threshold"] = below_threshold
		stats["below_threshold_runs"] = self._get_threshold_runs(below_threshold, periods_of_anomaly)
		stats["currently_in_below_threshold_run"] = self._is_metric_in_threshold_run_now(len(data), stats["below_threshold_runs"])
