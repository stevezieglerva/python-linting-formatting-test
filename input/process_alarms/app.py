...
def lambda_handler(event, context):
	...
	if need_to_check_outliers:
		...
		if metric_stats.get("currently_in_above_threshold_run", False) == False:
			...
			return({"processing_action" : f"skipped since not in threshold: {alarm_name}"})
		else:
			print(f"Metric {metric} is current in a threshold run")


	if 	alarm_type == ALARM_TYPE_NOT_FOUND:
		raise ValueError(f"Received alarm on {metric_namespace}/{metric} for {alarm_name} but that case is not handled. ")
		...
		log_error_details = log_error_samples(processing_local_miliseconds, log_group_name)
		message = f"Time:{processing_local}\n\nWe've detected an increase error keywords in the log for the {metric} metric."
		remeadiation = f"Look at the {metric} log in the AWS console for clues to the root cause."
	...
	alert = AlertMessage(alarm_name, subject, message, remeadiation, "", "", alarm_json, os.environ["EVENTS_BUCKET"], log_error_details)

	sns_output_topic = get_correct_sns_topic(alarm_name, log_error_details, os.environ["SNS_OUTPUT_TEXT"])
	...
	print("Finished.")
	result = {"processing_action" : "alarm processed", 
...
def get_alarm_type(metric_namespace, metric, alarm_name):
	...
	if metric_namespace == "AWS/Lambda" and metric == "Errors" and "BASELINE" in alarm_name:
...
def send_notifications(alert, sns_output_text):
	...
	sns.publish(TopicArn=sns_output_text, Subject=alert.subject, Message=alert.formatted_text)
...
def get_log_group_name(metricName, metricNamespace):
	...
	log_group_name = logs.describe_metric_filters( metricName=metricName, metricNamespace=metricNamespace)
	log_group_name = [logname["logGroupName"] for logname in log_group_name["metricFilters"]]
...
def log_error_samples(processing_local_miliseconds, logGroupName):
	sample_matches = []
	logs = boto3.client('logs')
	error_details = logs.filter_log_events(logGroupName=logGroupName, startTime=processing_local_miliseconds, filterPattern='?error ?Error ?ERROR ?exception ?Exception ?EXCEPTION', limit=3)
...
def need_to_check_for_outliers(alarm_name):
	if alarm_name == "ops-aws-ErrorMetricFilterAlarm-aws-lambda-ops-aws-fake-cw-errors-new":
		return False
...
def get_metric_name_from_alarm(alarm_json):
	if "MetricName" in alarm_json["Trigger"]:
		...
		metrics_stat = alarm_json["Trigger"]["Metrics"][position_index_with_metricstat].get("MetricStat", "")
		if metrics_stat == "":
			position_index_with_metricstat = 1
		metric = alarm_json["Trigger"]["Metrics"][position_index_with_metricstat]["MetricStat"]["Metric"]["MetricName"]
		metric_namespace = alarm_json["Trigger"]["Metrics"][position_index_with_metricstat]["MetricStat"]["Metric"]["Namespace"]
		return (metric_namespace, metric)
	else:
