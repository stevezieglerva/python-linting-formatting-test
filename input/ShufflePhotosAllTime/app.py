...
def lambda_handler(event, context):
	...
	total = merge_and_copy_portraits(this_month_portraits, len(this_month_portraits) - 2)
...
def copy_keys(keys_list):
	...
	for key in keys_list:
		...
		try:
			s3.meta.client.copy({"Bucket" : "svz-master-pictures-new", "Key" : orig_key}, "svz-master-pictures-new", new_key)
...
def delete_existing_shuffle_photos(max_pictures_in_bucket):
	...
	s3 = boto3.client("s3")
	list = s3.list_objects_v2(Bucket="svz-master-pictures-new", Prefix="nixplay")["Contents"]
...
def get_pictures_for_this_month():
	...
	db_results = d.query(TableName="master-pictures", IndexName="gsiYearMonth", KeyConditionExpression="#year = :year AND #month = :month", ExpressionAttributeNames={"#year" : "year", "#month" : "month"},  ExpressionAttributeValues={":year" : {"N" :  str(year)}, ":month" : {"N" :  str(month)}})
	keys = []
	portrait_keys = []
...
def get_random_pictures_for_month(month):
	...
	db_results = d.query(TableName="master-pictures", IndexName="gsiMonthRandom", KeyConditionExpression="#random = :random AND #month = :month", ExpressionAttributeNames={"#month" : "month", "#random" : "random"},  ExpressionAttributeValues={":random" : {"N" :  str(rand)}, ":month" : {"N" :  str(month)}})
	keys = []
	portrait_keys = []
	for i in db_results["Items"]:
		url = i["s3_url"]["S"]
...
def search(keywords):
	...
	db_results = db.query(TableName="master-pictures-keyword", KeyConditionExpression="keyword = :keyword", ExpressionAttributeValues={":keyword" : {"S" :  keywords}})
	file_list = []
